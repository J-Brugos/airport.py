...
def AssignNightGates(bcn, aircrafts):
    ...
    while i < len(aircrafts):
        ...
        if ac.origin == "" and ac.arrival == "":
            # Truco limpio: copiamos el destino al origen temporalmente
            # para que AssignGate pueda calcular si es Schengen o no
            ac.origin = ac.destination

            AssignGate(bcn, ac)

            # Restauramos el origen a vacío para dejar el objeto impecable
            ac.origin = ""
...
def FreeGate(bcn, id):
    t = 0
    # Recorremos las terminales
    while t < len(bcn.terminals):
        terminal = bcn.terminals[t]

        a = 0
        # Recorremos las zonas de embarque
        while a < len(terminal.boarding_areas):
            area = terminal.boarding_areas[a]

            g = 0
            # Recorremos las puertas de cada zona
            while g < len(area.gates):
                gate = area.gates[g]

                # Si encontramos el avión asignado a esta puerta
                if gate.aircraft_id == id:
                    gate.occupied = False
                    gate.aircraft_id = None
                    return 0  # Éxito: puerta liberada y salimos de la función

                g += 1
            a += 1
        t += 1

    return -1  # Código de error: el avión no se encontró en ninguna puerta
...
if __name__ == "__main__":
    ...
    bcn = LoadAirportStructure("Terminals.txt")
    if bcn != -1:
        print("--- PRUEBA NIGHT GATES ---")
        from aircraft import Aircraft

        # Un único avión de salida de prueba usando la clase Aircraft
        vuelo_test = Aircraft("ECLZN", "", "", "VLG", "LGMK", "00:19")

        AssignNightGates(bcn, [vuelo_test])

        # Verificamos si la primera puerta de la T1 se ha ocupado
        puerta = bcn.terminals[0].boarding_areas[0].gates[0]
        print(f"Puerta: {puerta.name} | Ocupada: {puerta.occupied} | Avión: {puerta.aircraft_id}")

        bcn = LoadAirportStructure("Terminals.txt")
        if bcn != -1:
            print("--- PRUEBA FREE GATE ---")
            # Forzamos la ocupación manual de la primera puerta para el test
            puerta_test = bcn.terminals[0].boarding_areas[0].gates[0]
            puerta_test.occupied = True
            puerta_test.aircraft_id = "ECLZN"

            # 1. Probamos a liberar un avión que SÍ existe
            res1 = FreeGate(bcn, "ECLZN")
            print(f"Liberar ECLZN -> Retorno: {res1} | ¿Sigue ocupada?: {puerta_test.occupied}")

            # 2. Probamos a liberar un avión que NO existe (debe dar error -1)
            res2 = FreeGate(bcn, "AVION_FANTASMA")
            print(f"Liberar inexistente -> Retorno: {res2} (Debe ser -1)")
